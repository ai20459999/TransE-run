...
import operator
import json
from transE import data_loader, entities2id, relations2id
import time
    ...
    def rank(self):
        ...
        for triple in self.test_triple:
            ...
            for entity in self.entity_dict.keys():  # triple是测试集的三元组，entity是导入的实体,keys()是0123456。。。等数字标签
                ...
                if self.isFit:
                    if [triple[0], triple[1], entity ] not in self.train_triple:  # 测试集的头实体、关系推理尾实体
    ...
    def relation_rank(self):  # 多数论文中并没有对于关系排名的介绍，是这个代码自己写的，实体的在下面，换成实体可能分数会降低
            ...
            for triple in self.test_triple:
                ...
                for r in self.relation_dict.keys():
                    if self.isFit and (triple[0], r, triple[2],) in self.train_triple:
                        continue
                    h_emb = self.entity_dict[triple[0]]
                ...
                rank_sorted = sorted(rank_dict.items(),
                                     key=operator.itemgetter(1))  # operator模块提供的itemgetter函数主要用于获取某一对象特定维度的数据
                # 这里根据rank_dict字典的分数来进行排名
                rank = 1
                for i in rank_sorted:
                    if triple[1] == i[1]:  # 预测对了，就跳出循环
                        break
                    rank += 1
                if rank < 10:
                    hits += 1
...
if __name__ == '__main__':
        _, _, train_triple = data_loader("../Test1/")  # 读取train_triple是为了filter训练方法

        entity_dict, relation_dict, test_triple = \
            dataloader("../res/Test_TransE_entity_30dim_batch5", "../res/Test_TransE_relation_30dim_batch5",
                       "../Test1/test1.txt")
