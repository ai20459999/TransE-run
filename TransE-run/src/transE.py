...
class TransE:
    ...
    def training_run(self, epochs=20, nbatches=300, out_file_title = ''):
        ...
        for epoch in range(epochs):
            ...
            for batch in range(nbatches):
                batch_samples = random.sample(self.triples, batch_size)#随机选取元素

                Tbatch = []#正确样本和负样本
                for sample in batch_samples:
                    corrupted_sample = copy.deepcopy(sample)#制作负样本，随机替换头尾实体
            ...
            if epoch % 5 == 0:
                with codecs.open(out_file_title +"TransE_entity", "w") as f_e:
                    for e in self.entities.keys():
                        f_e.write(str(e) + "\t")
                        f_e.write(str(list(self.entities[e])))
                        f_e.write("\n")
                with codecs.open(out_file_title +"TransE_relation", "w") as f_r:
                    for r in self.relations.keys():
                        f_r.write(str(r) + "\t")
                        f_r.write(str(list(self.relations[r])))
                        f_r.write("\n")
                with codecs.open(out_file_title +"result_temp", "a") as f_s:
...
if __name__ == '__main__':
    ...
    # modify by yourself
    transE = TransE(entity_set, relation_set, triple_list, embedding_dim=64, lr=0.01, margin=1.0, norm=2)
    transE.data_initialise()
    transE.training_run(out_file_title="Test_")
